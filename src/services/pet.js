...
class Pet {
  ...
  async createPet(userName, email, phoneNumber, petType, name, age, gender) {
    try {
      const res = await this.api.post("/pet/register", {
        userName,
        email,
        phoneNumber,
        petType,
  ...
  async updatePet(id, petName, age, gender) {
    try {
      const res = await this.api.post(`/pet/update/${id}`, {
        petName,
        age,
        gender,
      });
    ...
    try {
      ...
      throw new Error(
        error?.response?.data?.message ||
          error.message ||
          "Error occured while creating the medical record"
      );
    }
  }
  ...
  async updatePetMedicalInfo(id, data) {
    try {
      ...
      if (!response.data) {
        throw new Error("Error occured while updating the medical record");
      }
      return response.data;
    } catch (error) {
    ...
    throw new Error(
      error?.response?.data?.message ||
        error.message ||
        "Error occured while updating the medical record"
      );
    }
  }
  ...
  async removePet(id) {
    try {
      ...
      throw new Error(
        error?.response?.data?.message ||
          error.message ||
          "Failed to remove pet profile error occured "
      );
    }
  }
}
